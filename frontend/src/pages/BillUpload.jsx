...
const Icons = {
  UploadCloud: () => (
    <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-slate-400 group-hover:text-blue-600 transition-colors">
  ...
  return (
    <div className="w-full min-h-[calc(100vh-80px)] bg-slate-50 relative overflow-hidden font-sans text-slate-800 p-4 md:p-8 flex items-start justify-center">
      ...
      <div className="relative w-full max-w-[1400px] grid grid-cols-1 lg:grid-cols-12 gap-8 mt-4">
        ...
        {/* --- LEFT PANEL: UPLOAD & INGESTION MANAGEMENT (5 COLS) --- */}
        <div className="lg:col-span-5 bg-white/70 backdrop-blur-2xl border border-white/80 rounded-[2rem] p-6 md:p-8 shadow-[0_4px_30px_rgba(0,0,0,0.03)] flex flex-col justify-between">
          <div>
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 font-medium mt-1">Accepts only utility statement PDFs</p>
              </div>
            </div>
            ...
            {files.length > 0 && (
              <div className="mt-6 space-y-2.5">
                <div className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">Queue Stack</div>
                {files.map((f, i) => (
                  <div key={i} className={`flex items-center justify-between p-3.5 bg-white border rounded-xl shadow-sm transition-all ${i === selectedPreview ? 'border-blue-500 ring-2 ring-blue-50' : 'border-slate-100'}`}>
                    <div className="flex items-center gap-3 min-w-0">
                      <div className={`p-2 rounded-lg ${ocrResults[i] ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-50 text-slate-400'}`}>
                        <Icons.Document />
                      </div>
                      <span className="text-sm font-semibold text-slate-700 truncate w-48 md:w-64">{f.name}</span>
                    </div>
                    <button
          ...
          {error && (
            <div className="text-xs font-medium text-red-600 bg-red-50 px-4 py-2.5 rounded-lg border border-red-100 mt-4">{error}</div>
          )}
        </div>

        {/* --- RIGHT PANEL: LIVE IFRAME & STRUCTURED ATTRIBUTES (7 COLS) --- */}
        <div className="lg:col-span-7 bg-white/70 backdrop-blur-2xl border border-white/80 rounded-[2rem] p-6 md:p-8 shadow-[0_4px_30px_rgba(0,0,0,0.03)] flex flex-col">
          ...
          {files.length === 0 ? (
            <div className="flex flex-col items-center justify-center min-h-[450px] text-center flex-1">
              ...
              <h3 className="font-bold text-slate-400 text-sm uppercase tracking-widest">Inspection View Empty</h3>
              <p className="text-xs text-slate-400 mt-2 max-w-xs leading-relaxed">Upload statements to visually monitor real-time attribute isolation bounds.</p>
            </div>
          ) : (
            <div className="flex flex-col h-full justify-between flex-1">
              <div>
                ...
                {/* REAL-TIME ISOLATED FEATURE PROFILE DISPLAY */}
                <div>
                  <div className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-3">Extracted Profile Metadata</div>
                  
                  {extractedFields.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {extractedFields.map((f, i) => (
                        <div key={i} className="p-3.5 bg-white border border-slate-100 rounded-xl shadow-sm flex flex-col justify-center">
                    ...
                    <div className="p-8 text-center bg-slate-50/50 border border-slate-200 border-dashed rounded-xl">
                      <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">
                        {uploading ? "Extracting model tensors..." : "Awaiting Pipeline Execution"}
                      </p>
                    </div>
                  )}
                </div>
