...
export default function KYC({ token, go }) {
  ...
  const sendCode = async () => {
    ...
    try {
      // Backend validates the NIC, generates the OTP and emails it.
      // The code is never exposed to the client.
      const result = await axios.post(`${API}/api/kyc/verify-nic`, { nic_number: nic }, { headers });
      setEmail(result.data.email);
      if (result.data?.delivery === "server-log") {
        setNotice("Email delivery isn't configured — find the code in the backend logs.");
      }
      setStage("otp");
    } catch (requestError) {
    ...
    <div className="min-h-screen bg-white">
      <div className="w-full overflow-hidden bg-white md:grid md:min-h-screen md:grid-cols-2">
        ...
        <section className="flex min-h-[52vh] flex-col justify-between p-8 md:min-h-screen md:p-12">
          ...
          {stage === "nic" ? <NicForm nic={nic} setNic={setNic} sendCode={sendCode} loading={loading} error={error} /> : <OtpForm email={email} otp={otp} setOtp={setOtp} verify={verify} loading={loading} error={error} notice={notice} resend={sendCode} />}
        </section>
      </div>
    </div>
  );
...
function NicForm({ nic, setNic, sendCode, loading, error }) { return <div className="mx-auto my-10 w-full max-w-md"><span className="rounded-full border border-blue-100 bg-blue-50 px-3 py-1 text-xs font-bold uppercase tracking-wider text-[#004fc5]">Identity verification</span><h1 className="mt-5 text-3xl font-extrabold tracking-tight text-slate-900">Verify your identity</h1><p className="mt-2 text-sm leading-relaxed text-slate-500">Enter your National Identity Card number to receive a six-digit code at your registered email.</p><label htmlFor="nic" className="mt-8 mb-2 block text-xs font-bold uppercase tracking-wider text-slate-700">NIC number</label><input id="nic" value={nic} onChange={(event) => setNic(event.target.value)} placeholder="199012345678 or 900123456V" className="w-full rounded-xl border border-slate-200 px-4 py-3.5 font-mono text-sm outline-none focus:border-[#004fc5] focus:ring-4 focus:ring-blue-100" /><p className="mt-1.5 text-[11px] text-slate-400">Accepts classic 9-digit + V/X or modern 12-digit format.</p>{error && <Error text={error} />}<button disabled={loading || !nic} onClick={sendCode} className="mt-6 flex h-12 w-full items-center justify-center rounded-full bg-[#004fc5] px-6 py-3.5 text-sm font-bold tracking-wide text-white shadow-lg shadow-blue-500/25 transition hover:bg-[#003a94] disabled:opacity-60">{loading ? "Sending code…" : "Send verification code →"}</button></div>; }
function OtpForm({ email, otp, setOtp, verify, loading, error, notice, resend }) { return <div className="mx-auto my-10 w-full max-w-md"><h1 className="text-3xl font-extrabold tracking-tight text-slate-900">Check your email</h1><p className="mt-2 text-sm leading-relaxed text-slate-500">We sent a 6-digit confirmation code to <span className="font-semibold text-slate-800">{email}</span>.</p><div className="mt-8"><OtpInput value={otp} onChange={setOtp} onComplete={setOtp} /></div>{notice && <p className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs font-medium text-amber-800">{notice}</p>}<div className="mt-5 rounded-xl border border-slate-100 bg-slate-50 p-4 text-xs"><div className="flex justify-between text-slate-500"><span>Didn't receive the code?</span><button onClick={resend} disabled={loading} className="font-bold text-[#004fc5] hover:underline disabled:opacity-50">Resend code</button></div><p className="mt-3 text-slate-500">Verification is sent to your registered email address.</p></div>{error && <Error text={error} />}<button disabled={loading || otp.length !== 6} onClick={verify} className="mt-6 flex h-12 w-full items-center justify-center rounded-full bg-[#004fc5] px-6 py-3.5 text-sm font-bold tracking-wide text-white shadow-lg shadow-blue-500/25 transition hover:bg-[#003a94] disabled:opacity-60">{loading ? "Verifying…" : "Verify & Continue →"}</button></div>; }
