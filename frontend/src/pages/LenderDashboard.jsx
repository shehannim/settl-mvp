...
export default function LenderDashboard({ go }) {
  ...
  const search = (e) => {
    ...
    if (!found) {
      setQueryError(`No applicant found for “${query.trim()}”. Try a Settl ID from the book below.`);
      return;
    }
    ...
    <div className="min-h-[calc(100vh-72px)] bg-[#f8f9ff] px-4 py-6 text-slate-900 sm:px-6 lg:px-8 font-sans">
      <main className="mx-auto max-w-[1180px] space-y-6">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_2px_12px_-2px_rgba(15,23,42,0.04)]">
          ...
          <p className="mt-0.5 text-xs text-slate-500">Look up by Settl ID (e.g. STL-2026-A41F9C) or applicant name.</p>
          <form onSubmit={search} className="mt-4 flex flex-col sm:flex-row gap-2.5">
            ...
            />
            <button type="submit" className="rounded-xl bg-[#004fc5] px-6 py-3 text-sm font-bold text-white hover:bg-[#003a94]">
              Query score
            </button>
          </form>
          {queryError && (
            ...
            <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50/60 p-5">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <p className="font-mono text-xs text-slate-500">{result.applicant.settl_id} · {result.applicant.sources} sources · {result.applicant.model_version} · scored {result.applicant.scored_at}</p>
                </div>
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="space-y-2">
                    {result.applicant.top_positive_factors.map((f, i) => (
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {result.applicant.top_negative_factors.map((f, i) => (
                      <div key={i} className="rounded-xl bg-white border border-slate-100 p-3">
