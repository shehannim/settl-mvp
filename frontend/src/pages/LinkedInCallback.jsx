...
const Icons = {
  Spinner: () => (
    <div className="w-12 h-12 border-4 border-slate-200 border-t-[#0A66C2] rounded-full animate-spin shadow-sm"></div>
  ),
  Success: () => (
...
export default function LinkedInCallback({ go }) {
      ...
      <div className="relative w-full max-w-md bg-white/80 backdrop-blur-2xl border border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-[2rem] p-10 text-center flex flex-col items-center">
        {status === "processing" && (
          <div className="flex flex-col items-center animate-in fade-in duration-500">
            ...
            <p className="text-sm font-medium text-slate-500">
              Confirming profile with LinkedIn...
            </p>
          </div>
        )}
        ...
        {status === "error" && (
          <div className="flex flex-col items-center animate-in fade-in duration-300 w-full">
            ...
            <button
              onClick={() => go && go("linkedin-connect")}
