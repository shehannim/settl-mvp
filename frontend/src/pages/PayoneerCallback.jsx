...
const Icons = {
  Spinner: () => (
    <div className="w-12 h-12 border-4 border-slate-200 border-t-orange-500 rounded-full animate-spin shadow-sm"></div>
  ),
  Success: () => (
...
export default function PayoneerCallback({ go }) {
  ...
  useEffect(() => {
    ...
    if (!code) {
      setStatus("error");
      setError("No authorization code received from Payoneer.");
      return;
    }
    ...
    fetch(
      ...
      .then(async (res) => {
        if (!res.ok) {
          ...
          throw new Error(data.detail || `Payoneer callback failed (${res.status})`);
        }
        ...
        if (!src.ok) throw new Error("Payoneer approved, but your session expired before the link could be confirmed. Sign in and reconnect.");
        const list = (await src.json().catch(() => ({}))).sources || [];
        if (!list.some((s) => s.source === "payoneer")) {
          throw new Error("Payoneer approved, but the link wasn't saved. Reconnect once — if it repeats, the backend database write is failing.");
        }
        setStatus("success");
      ...
      .catch((err) => {
        console.error(err);
        setStatus("error");
        setError(err.message || "Failed to establish secure connection with Payoneer.");
      });
  ...
  return (
    <div className="w-full min-h-[calc(100vh-80px)] bg-slate-50 relative overflow-hidden font-sans text-slate-800 p-4 flex items-center justify-center">
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[40%] h-[40%] bg-orange-200/40 rounded-full blur-[100px] pointer-events-none" />
      ...
      <div className="relative w-full max-w-md bg-white/80 backdrop-blur-2xl border border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-[2rem] p-10 text-center flex flex-col items-center">
        ...
        {status === "error" && (
          <div className="flex flex-col items-center animate-in fade-in duration-300 w-full">
            <div className="mb-6"><Icons.Error /></div>
            <h2 className="text-2xl font-extrabold tracking-tight text-slate-900 mb-2">
              Connection Failed
            </h2>
